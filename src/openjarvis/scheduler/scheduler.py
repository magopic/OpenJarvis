...
class TaskScheduler:
    ...
    def create_task(
        ...
        *task_id* lets a caller pin a deterministic id (e.g. Operators use
        ``operator:{operator_id}``) instead of the random one generated by
        default, so the row can be looked up/updated later without a
        separate save under a second id.
        """
        task = ScheduledTask(
